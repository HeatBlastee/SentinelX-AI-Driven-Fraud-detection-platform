...
const FraudCard = ({ fraud }) => {
  ...
  return (
    <div className="SentinelX-panel-soft rounded-2xl p-4 border border-slate-800/80 hover:border-cyan-500/40 transition-all">
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          <AlertTriangle className="w-5 h-5 text-red-400" />
          <div>
            <span className="font-semibold text-white">{fraud.type}</span>
            <p className="text-xs text-slate-400 SentinelX-mono">{fraud.transactionId}</p>
          </div>
        </div>
      ...
      <div className="space-y-2 text-sm mb-3">
        ...
        <div className="flex justify-between">
          ...
          <span className="text-slate-300 SentinelX-mono text-xs">{fraud.customerId}</span>
        </div>
      </div>
